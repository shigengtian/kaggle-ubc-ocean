...
if __name__ == "__main__":
    ...
    train_tile_df = pd.read_csv("dataset/tile_df.csv")

    train_tile_df = train_tile_df[train_tile_df["mask_ratio"] > 0.8].reset_index(
        drop=True
    )
    
    print(len(train_tile_df))

    train_tile_df["image_id"] = train_tile_df["tile_path"].apply(
        lambda x: x.split("/")[-1].split(".")[0].split("_")[0]
    )
    # print(train_tile_df.head())
